...
import dataLoader_oneless, miscFunctions, oscfit_default_values_oneless_tom
from likelihoods import likelihoods
reload(dataLoader_oneless)
    ...
    def printMinStepHeaders(self, fsettings):
        if self.printMode < 0:
            if len(self.print_keys) == 0:
                for key in oscfit_default_values_oneless_tom.fit_keys:
...
#                         nu_frac12 = None,
        ...
        kwargs = locals()
        for iKey in oscfit_default_values_oneless_tom.fit_keys:
...
#            norm_12 = np.abs(norm_12)
        ...
        norm_7 = nu_events*(1-nu_frac1-nu_frac2-nu_frac3-nu_frac4-nu_frac5-nu_frac6)/(np.sum(neutrinos[5])*norm_nu)
        if norm_7 < 0:
            print 'oscFit: Norm 12 as we now call it, was calculated to be negative! using absolute value'
            norm_7 = np.abs(norm_7)
...
#	print '                                                 '
        nu_norms = [norm_1, norm_2, norm_3, norm_4,norm_5, norm_6, norm_7, 0.]
...
#	print '                                                   '
	...
	atmmu_norm_error_fixed - scaling factor.  For 'standard behavior' set to a negiative value and this parameter will 
        ...
        if len(data_loaders) == 0:
            ...
            exit()
        miscFunctions.checkDictParams(oscfit_default_values_oneless_tom.default_fit_settings,
                                      fit_settings, 'oscFit')
        fsettings = deepcopy(oscfit_default_values_oneless_tom.default_fit_settings)
        fsettings.update(fit_settings)
        ferrors = deepcopy(oscfit_default_values_oneless_tom.default_fit_errors)
        ferrors.update(fit_errors)
        miscFunctions.checkDictParams(oscfit_default_values_oneless_tom.default_fit_limits,
                                      fit_limits, 'oscFit')
        flimits = deepcopy(oscfit_default_values_oneless_tom.default_fit_limits)
        miscFunctions.checkDictParams(oscfit_default_values_oneless_tom.default_fit_settings, 
                                      fit_priors, 'oscFit')
        self.fit_priors = deepcopy(oscfit_default_values_oneless_tom.default_fit_priors)
        ...
        starting_eval_dict = {}
        for one_key in oscfit_default_values_oneless_tom.fit_keys:
	...
	print starting_eval_dict
        ...
        if fsettings["minimizer"].lower() == "migrad":        
            ...
            for one_var in oscfit_default_values_oneless_tom.fit_keys:
		...
		#results['errors_valid'] = m.is_valid       #Validity of minos error value. This means lower_valid and upper_valid
            ...
            tArgs   = deepcopy(oscfit_default_values_oneless_tom.fit_keys)
            tDirec  = ferrors
            tLimits = flimits
...
#                                (np.sum(neutrinos[11])*norm_nu_secs))]
			...
			np.abs(nu_events*
                                     (1-results['nu_frac1']-results['nu_frac2']-results['nu_frac3']-results['nu_frac4']-
                                     results['nu_frac5']-results['nu_frac6'])/\
