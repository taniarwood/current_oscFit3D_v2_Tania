...
import oscfit_default_values_oneless
reload(oscfit_default_values_oneless)
...
class dataLoader(object):
    ...
    def countMCevents_seperate(self, etype = 'CC'):
	return len(self.nue['baseline'][etype][self.hist_obs[0]])    

    def getWeightsDetector(self):
        self.calculateEventParams()
        return self.weights_detector
...
#LET params['nu_pi_scale'] be one 1.0 (so we have the right amount of flux
        # NuE produced as NuMu
        nue_numu_ccflux_init   = (nue_cc[self.wmu]+1.0*nue_cc[self.wmu_pi])*\
  ...
  #      numu_nue_ccflux_init   = numu_cc[self.we]*params['norm_e']*numu_cc['energy']**params['gamma']*\
  ...
  #                          sf.modRatioUpHor_NuE(numu_cc, params['uphor_ratio'])
        ...
        numu_numu_ccflux_init  = (numu_cc[self.wmu]+1.0*numu_cc[self.wmu_pi])*\
 ...
 #                             sf.modRatioUpHor_NuE(nutau_cc, params['uphor_ratio'])
	...
	#print nue_numu_ccflux_init
	#print '......................'
	#print numu_numu_ccflux_init 
	#print '....................'
	#print nutau_numu_ccflux_init 
	...
	# as a check and tool for reference, let  params['nu_pi_scale'] = 1.0 so that we have the correct 
	#amount of starting fluxes to look at 
        
	# NuE produced as NuE
        nue_nue_ccflux_init    = (nue_cc[self.we] + 1.0*nue_cc[self.we_pi])#*\
        
	# NuE produced as NuMu
        nue_numu_ccflux_init    = (nue_cc[self.wmu]+1.0*nue_cc[self.wmu_pi])#*\

        # NuMu produced as NuE
        numu_nue_ccflux_init   = (numu_cc[self.we] + params['nu_pi_scale']*numu_cc[self.we_pi])#*\

        # NuMu produced as NuMu
        numu_numu_ccflux_init  = (numu_cc[self.wmu]+ 1.0*numu_cc[self.wmu_pi])#*\

        # NuTau produced as NuE
        nutau_nue_ccflux_init    = (nutau_cc[self.we] + 1.0*nutau_cc[self.we_pi])#*\
        ...
        # JP adding the flux flattener here  *Note! this does not get used if you don't call the flattener module in your loader ...
	# but Tania code below does get used no matter what you call... think on this. Would that make a difference?
	...
	# Tania added, things coming from NuE need to be corrected also now (e, mu, tau, nc) 
...
#Tania added it again (ee) in order to do some verification tests
	...
	if np.sum(numu_ncweight) != 0.0:
		numu_ncweight *= np.sum(numu_ncweight_init)/np.sum(numu_ncweight)
	#NuE
        if np.sum(nue_nue_ccflux) != 0.0:
                nue_nue_ccflux *= np.sum(nue_nue_ccflux_init)/np.sum(nue_nue_ccflux)

        if np.sum(numu_nue_ccflux) != 0.0:
                numu_nue_ccflux *= np.sum(numu_nue_ccflux_init)/np.sum(numu_nue_ccflux)

        if np.sum(nutau_nue_ccflux) != 0.0:
                nutau_nue_ccflux *= np.sum(nutau_nue_ccflux_init)/np.sum(nutau_nue_ccflux)

        if np.sum(nue_ncweight) != 0.0:
                nue_ncweight *= np.sum(nue_ncweight_init)/np.sum(nue_ncweight)
    ...
    def __init__(self, 
                 ...
                 user         = 'quickstart', 
                 LEaxis       = [],
                 weight_keys  = ['weight_e', 'weight_e_pi', 'weight_mu', 'weight_mu_pi'], 
                 break_energy = -1,
                 extra_cuts   = {},
                 table_nbins  = -1,
	...
	self.wmu             = weight_keys[2]
        ...
        self.default_data_settings = oscfit_default_values_oneless.default_data_settings
