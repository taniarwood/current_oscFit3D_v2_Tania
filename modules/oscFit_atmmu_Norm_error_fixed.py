...
#now nue  (8 bins), numu was 9 bins
	...
	norm_10 = nu_events*nu_frac10/(np.sum(neutrinos[9])*norm_nu)
        ...
        norm_13 = nu_events*nu_frac13/(np.sum(neutrinos[12])*norm_nu)




  #      norm_14 = nu_events*nu_frac6/(np.sum(neutrinos[13])*norm_nu)
  #      norm_15 = nu_events*nu_frac7/(np.sum(neutrinos[14])*norm_nu)
  #      norm_16 = nu_events*nu_frac8/(np.sum(neutrinos[15])*norm_nu)



	norm_14 = nu_events*(1-nu_frac1-nu_frac2-nu_frac3-nu_frac4-nu_frac5-nu_frac6-nu_frac7-nu_frac8- nu_frac9-nu_frac10 - nu_frac11 - nu_frac12-nu_frac13  )/(np.sum(neutrinos[12])*norm_nu)
        if norm_14 < 0:
            print 'oscFit: Norm 9 , or 17 as we now call it, was calculated to be negative! using absolute value'
            norm_14 = np.abs(norm_14)
...
#        norm_8 = np.abs(norm_8)
      ...
      # Nine bins plus 8 bins = 17 bins, minus one bin to wrap around 
        ...
        nu_norms = [norm_1, norm_2, norm_3, norm_4,norm_5, norm_6, norm_7, norm_8, norm_9,norm_10, norm_11, norm_12, norm_13,norm_14,  0.]
        ...
        nu_norms  = [nu_events*results['nu_frac1']/(np.sum(neutrinos[0])*norm_nu_secs),
		     ...
		     nu_events*results['nu_frac4']/(np.sum(neutrinos[3])*norm_nu_secs),
                    ...
                    nu_events*results['nu_frac10']/(np.sum(neutrinos[9])*norm_nu_secs),
                     ...
                     nu_events*results['nu_frac13']/(np.sum(neutrinos[12])*norm_nu_secs),
        #             nu_events*results['nu_frac14']/(np.sum(neutrinos[13])*norm_nu_secs),
        #             nu_events*results['nu_frac15']/(np.sum(neutrinos[14])*norm_nu_secs),
        #             nu_events*results['nu_frac16']/(np.sum(neutrinos[15])*norm_nu_secs),
		     ...
		     np.abs(nu_events*
                            (1-results['nu_frac1']-results['nu_frac2']-results['nu_frac3']-results['nu_frac4']-
                             results['nu_frac5']-results['nu_frac6']-results['nu_frac7']-results['nu_frac8'] -results['nu_frac9'] -results['nu_frac10']-results['nu_frac11']-results['nu_frac12']-results['nu_frac13']  )/\
