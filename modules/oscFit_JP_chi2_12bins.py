...
import dataLoader_oneless, miscFunctions, oscfit_default_values_oneless
    ...
    def printMinStepHeaders(self, fsettings):
        if self.printMode < 0:
            if len(self.print_keys) == 0:
                for key in oscfit_default_values_oneless.fit_keys:
...
#                         nu_frac12 = None,
        ...
        kwargs = locals()
        for iKey in oscfit_default_values_oneless.fit_keys:
...
#	print '                                                   '
	...
	atmmu_norm_error_fixed - scaling factor.  For 'standard behavior' set to a negiative value and this parameter will 
        ...
        if len(data_loaders) == 0:
            ...
            exit()
        miscFunctions.checkDictParams(oscfit_default_values_oneless.default_fit_settings,
                                      fit_settings, 'oscFit')
        fsettings = deepcopy(oscfit_default_values_oneless.default_fit_settings)
        fsettings.update(fit_settings)
        ferrors = deepcopy(oscfit_default_values_oneless.default_fit_errors)
        ferrors.update(fit_errors)
        miscFunctions.checkDictParams(oscfit_default_values_oneless.default_fit_limits,
                                      fit_limits, 'oscFit')
        flimits = deepcopy(oscfit_default_values_oneless.default_fit_limits)
        miscFunctions.checkDictParams(oscfit_default_values_oneless.default_fit_settings, 
                                      fit_priors, 'oscFit')
        self.fit_priors = deepcopy(oscfit_default_values_oneless.default_fit_priors)
        ...
        starting_eval_dict = {}
        for one_key in oscfit_default_values_oneless.fit_keys:
	...
	print starting_eval_dict
        ...
        if fsettings["minimizer"].lower() == "migrad":        
            ...
            for one_var in oscfit_default_values_oneless.fit_keys:
		...
		#results['errors_valid'] = m.is_valid       #Validity of minos error value. This means lower_valid and upper_valid
            ...
            tArgs   = deepcopy(oscfit_default_values_oneless.fit_keys)
            tDirec  = ferrors
            tLimits = flimits
