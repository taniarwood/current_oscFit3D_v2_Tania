...
#import dataLoader_oneless, miscFunctions, oscfit_default_values_onesless
import dataLoader_corrected_weight_e_pi_modelt,  miscFunctions, oscfit_default_values_modelt

from likelihoods import likelihoods
reload(dataLoader_corrected_weight_e_pi_modelt)
    ...
    def printMinStepHeaders(self, fsettings):
        if self.printMode < 0:
            if len(self.print_keys) == 0:
                for key in oscfit_default_values_modelt.fit_keys:
...
#                         nu_frac12 = None,
        ...
        kwargs = locals()
        for iKey in oscfit_default_values_modelt.fit_keys:
        ...
        for index, one_loader in enumerate(self.data_loaders):
            ...
            if self.fit_function == 'chi_squared':
              ...
              #nu_norms = [norm_1, norm_2,  0.]

        nu_norms = [norm_1, norm_2, norm_3, norm_4, 0.]
...
##  This is what is used in the unfolding (Below)
        norm_nu = expected_events*(1-atmmu_f-noise_f)/np.sum([np.sum(neutrinos[i])*nu_norms[i]
...
#	norm_12 = nu_events*(1-nu_frac1-nu_frac2-nu_frac3-nu_frac4-nu_frac5-nu_frac6-nu_frac7-nu_frac8-
 #                            nu_frac9-nu_frac10-nu_frac11)/(np.sum(neutrinos[11])*norm_nu)
 #       if norm_12 < 0:
#            print 'oscFit: Norm 12 as we now call it, was calculated to be negative! using absolute value'
#            norm_12 = np.abs(norm_12)
#
...
##	print '                                                 '
#        nu_norms = [norm_1, norm_2, norm_3, norm_4,norm_5, norm_6, norm_7, norm_8, norm_9,norm_10, norm_11, norm_12, 0.]
...
##	print '                                                   '
	...
	atmmu_norm_error_fixed - scaling factor.  For 'standard behavior' set to a negiative value and this parameter will 
        ...
        if len(data_loaders) == 0:
            ...
            exit()
        miscFunctions.checkDictParams(oscfit_default_values_modelt.default_fit_settings,
                                      fit_settings, 'oscFit')
        fsettings = deepcopy(oscfit_default_values_modelt.default_fit_settings)
        fsettings.update(fit_settings)
        ferrors = deepcopy(oscfit_default_values_modelt.default_fit_errors)
        ferrors.update(fit_errors)
        miscFunctions.checkDictParams(oscfit_default_values_modelt.default_fit_limits,
                                      fit_limits, 'oscFit')
        flimits = deepcopy(oscfit_default_values_modelt.default_fit_limits)
        miscFunctions.checkDictParams(oscfit_default_values_modelt.default_fit_settings, 
                                      fit_priors, 'oscFit')
        self.fit_priors = deepcopy(oscfit_default_values_modelt.default_fit_priors)
        ...
        starting_eval_dict = {}
        for one_key in oscfit_default_values_modelt.fit_keys:
	...
	print starting_eval_dict
        ...
        if fsettings["minimizer"].lower() == "migrad":        
            ...
            for one_var in oscfit_default_values_modelt.fit_keys:
            ...
            while (not good_fit) and (fit_counter < 3):
                ...
                try:
                    ...
                    m = iminuit.Minuit(self.llhFunction, **kwargs)
                    m.strategy  = 1
                    m.errordef  = 1.0 #1.0  #  This should be set as 0.5 for -log(llh) and 1.0 for a chi2 
                    m.tol       = tol  #http://iminuit.readthedocs.io/en/latest/api.html?highlight=errordef
                    # Minimize
		    ...
		    #good_fit = True
                    if m.migrad_ok():
			good_fit = True
			print 'm.migrad_ok() True was acheived'
                except:
            ...
            if good_fit and store_fit_details and hesse_errors:
                ...
                results['errors']      = m.errors   #changed from m.errors (Which is not recommended) to get_merrors 
		#results['errors_valid'] = m.is_valid       #Validity of minos error value. This means lower_valid and upper_valid
							   #http://iminuit.readthedocs.io/en/latest/api.html?highlight=fit%20status#minos-error-struct 
            ...
            tArgs   = deepcopy(oscfit_default_values_modelt.fit_keys)
            tDirec  = ferrors
            tLimits = flimits
        ...
        nu_norms = [results['norm_1'],results['norm_2'],results['norm_3'], results['norm_4'], 0.]
...
#        results['norm_nu'] = nu_events/(np.sum([np.sum(neutrinos[i])for i in range(len(neutrinos))])*\
 #                                       self.data_loaders[0].sec2years)
        ...
        results['norm_nu'] = expected_events_fit*(1-results['atmmu_f']-results['noise_f'])/\
                             (np.sum([np.sum(neutrinos[i])*nu_norms[i] for i in range(len(neutrinos))])*\
                                     self.data_loaders[0].sec2years)
...
#                             results['nu_frac9'] -results['nu_frac10']-results['nu_frac11'])/\
#                                (np.sum(neutrinos[11])*norm_nu_secs))]
